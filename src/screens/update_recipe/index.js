...
const UpdateRecipe = ({navigation}) => {
  ...
  const submitRecipe = () => {
    ...
    axios
      .put('http://10.0.2.2:3000/update/49', data, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      })
      .then(response => {
        console.log('Resep berhasil diubah:', response.data);
        Alert.alert('update recipe Successful');
      })
      .catch(error => {
        // Tangani error jika gagal
        console.error('Gagal mengubah resep:', data.image, error);
      });
  };
